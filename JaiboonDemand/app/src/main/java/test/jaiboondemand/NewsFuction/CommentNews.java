...
public class CommentNews extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar = (Toolbar) findViewById(R.id.toolbar_main_comment_news);
        setSupportActionBar(toolbar);
        setTitle("Comment");

        recyclerView = (RecyclerView) findViewById(R.id.comment_recycleview_news);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setHasFixedSize(true);

        post_key = getIntent().getExtras().getString("PostID");

        mData = FirebaseDatabase.getInstance().getReference().child("News").child(post_key);
        mUser = FirebaseDatabase.getInstance().getReference().child("Users");

        mCommentEditTextView = (EditText) findViewById(R.id.et_comment_news);
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<CommentFirebase,CommentHolder> adapter = new FirebaseRecyclerAdapter<CommentFirebase, CommentHolder>(
                CommentFirebase.class,
                R.layout.row_comment,
                CommentHolder.class,
                mData.child("Comment")
        ) {
            @Override
            protected void populateViewHolder(final CommentHolder viewHolder, CommentFirebase model, int position) {
              try{
